...
import static drift.util.TestConstants.CHAMPIONSHIP_STAGE_ID

class ChampionshipStageRepositoryTest extends BaseFunctionalTest {

    def 'should throw exception during saving if [id] field is not set'() {
        when:
        championshipStageRepository.saveAndFlush(ChampionshipStage.builder().build())
    ...
    def 'should throw exception during saving if required fields are not set'() {
        given:
        def requiredFields = [
                'ownerId', 'championshipId', 'name', 'duration', 'startTimestamp',
                'location', 'description', 'participationInfo', 'attempts', 'omt'
        ]

        when:
        championshipStageRepository.saveAndFlush(ChampionshipStage.builder().id(CHAMPIONSHIP_STAGE_ID).build())
